...
/**
 * https://github.com/IBM/science-result-extractor/blob/master/nlpLeaderboard/src/main/java/com/ibm/sre/pdfparser/NLPLeaderboardTable.java
 * @author yhou
...
//                            System.err.println(LayoutTokensUtil.toText(c1.lt) + "---" + LayoutTokensUtil.toText(c2.lt));
        ...
        for (Map.Entry<Double, List<TableCell>> item : columnOfRows.entrySet()) {
        ...
        for (Map.Entry<Double, List<TableCell>> item : rowsOfColumn.entrySet()) {
            if (item.getValue().size() == 1) {
