...
DATA_DIR=../data/paperwithcode/new/jar/80Neg600unk/twofoldwithunk/fold1
...
python run_sci_classifier.py --use_tpu=False --do_train=false --do_eval=false --do_predict=true --task_name=sci --data_dir=$DATA_DIR --output_dir=$Out_DIR --predict_dir=$Out_DIR --model_dir=$Out_DIR --uncased=False --model_config_path=$XLN_DIR/xlnet_config.json --spiece_model_file=$XLN_DIR/spiece.model --init_checkpoint=$XLN_DIR/xlnet_model.ckpt --max_seq_length=512 --train_batch_size=32 --predict_batch_size=16 --num_hosts=1 --num_core_per_host=1 --learning_rate=1e-5 --train_steps=5000 --warmup_steps=500 --save_steps=500 --iterations=1000