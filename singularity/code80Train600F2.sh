...
Out_DIR=./model/80Neg600unkFold2BertBatch16
if [ -d "$Out_DIR" ]; then
...
python run_classifier_sci.py --do_train=true --do_eval=true --do_predict=false --data_dir=$DATA_DIR --task_name=sci --vocab_file=$BERT_DIR/vocab.txt --bert_config_file=$BERT_DIR/bert_config.json --init_checkpoint=$BERT_DIR/bert_model.ckpt --output_dir=$Out_DIR --max_seq_length=512 --train_batch_size=16 --eval_batch_size=16 --predict_batch_size=6